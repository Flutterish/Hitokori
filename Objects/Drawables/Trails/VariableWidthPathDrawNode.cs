...
namespace osu.Framework.Graphics.Lines {
    public partial class VariableWidthPath {
        private class VariableWidthPathDrawNode : DrawNode {
            public const int MAX_RES = 24;

            protected new VariableWidthPath Source => (VariableWidthPath)base.Source;
            ...
            public VariableWidthPathDrawNode ( VariableWidthPath source )
                : base( source ) {
            }
            ...
            private void addLineCap ( Vector2 origin, float theta, float thetaDiff, RectangleF texRect, float radius ) {
            ...
            private void addLineQuads ( int index, Line line, RectangleF texRect ) {
                var startRadius = Source.RadiusAtVertice( index );
                var endRadius = Source.RadiusAtVertice( index + 1 );

                Vector2 ortho = line.OrthogonalDirection;
                Line lineLeft = new Line( line.StartPoint + ortho * startRadius, line.EndPoint + ortho * endRadius );
                Line lineRight = new Line( line.StartPoint - ortho * startRadius, line.EndPoint - ortho * endRadius );
            ...
            private void updateVertexBuffer () {
                ...
                addLineCap( line.StartPoint, theta + MathF.PI, MathF.PI, texRect, Source.startPathRadius );

                for ( int i = 1; i < segments.Count; ++i ) {
                    Line nextLine = segments[ i ];
                    float nextTheta = nextLine.Theta;
                    addLineCap( line.EndPoint, theta, nextTheta - theta, texRect, Source.RadiusAtVertice( i ) );
                
                    line = nextLine;
                    theta = nextTheta;
                }

                addLineCap( line.EndPoint, theta, MathF.PI, texRect, Source.endPathRadius );

                for ( int i = 0; i < segments.Count; i++ )
                    addLineQuads( i, segments[ i ], texRect );
            }
